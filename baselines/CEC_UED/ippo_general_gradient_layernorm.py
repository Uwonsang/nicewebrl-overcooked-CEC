...
ACTOR_TRUNK_KEYS = (
    ...
    "SharedLayerNorm_0", "SharedLayerNorm_1",
)
VALUE_TRUNK_KEYS = (
    "Conv_0", "Conv_1", "Dense_0", "Dense_1", "ScannedRNN_0",
...
class ActorCriticRNN(nn.Module):
    ...
    def __call__(self, hidden, x):
        ...
        if self.config["CONV_NET"]:
            ...
            embedding = nn.LayerNorm(name="SharedLayerNorm_0")(embedding)
            ...
            embedding = nn.LayerNorm(name="SharedLayerNorm_1")(embedding)
            embedding = nn.relu(embedding)
...
def main(config):
    ...
    config["model_name"] = "CEC_LAYERNORM"
    ...
    filepath_base = f"ckpts/ippo_layernorm/{config['ENV_NAME']}"
    if config["ENV_NAME"] == "overcooked":
    ...
    if wandb_resume_id:
        ...
        wandb.init(
            entity=config["ENTITY"],
            project=config["PROJECT"],
            tags=["IPPO", "RNN", "SP", "LayerNorm"],
            config=config,
            mode=config["WANDB_MODE"],
            name=f"CEC_gradient_layernorm_{layout_name}_seed{config['SEED']}"
        )
