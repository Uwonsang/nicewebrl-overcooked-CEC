...
AUXILIARY_PARAM_PREFIXES = (
    ...
    "order_classifier_output",
)


def _idaac_fc_dim(config):
    return int(config.get("IDAAC_FC_DIM_SIZE", IDAAC_MATCHED_FC_DIM))


def _idaac_policy_hidden_dims(config):
    return tuple(
        int(width)
        for width in config.get(
            "IDAAC_POLICY_HIDDEN_DIMS",
            IDAAC_MATCHED_POLICY_HIDDEN_DIMS,
        )
    )


def _idaac_value_hidden_dims(config):
    return tuple(
        int(width)
        for width in config.get(
            "IDAAC_VALUE_HIDDEN_DIMS",
            IDAAC_MATCHED_VALUE_HIDDEN_DIMS,
        )
    )


def _count_policy_value_parameters(variables):
    ...
    return sum(
        ...
        if not any(
            str(path_component).startswith(prefix)
            for path_component in path
            for prefix in AUXILIARY_PARAM_PREFIXES
        )
    )
...
class RecurrentFeatureTrunk(nn.Module):
    ...
    def __call__(self, hidden, obs, dones):
        ...
        fc_dim = _idaac_fc_dim(self.config)
...
class ActorCriticRNN(nn.Module):
    ...
    def __call__(self, hidden, x, return_auxiliary=False,
        ...
        policy_hidden_dims = _idaac_policy_hidden_dims(self.config)
        value_hidden_dims = _idaac_value_hidden_dims(self.config)
...
def make_train(
    ...
    # This variant's architecture is fixed in code. The shared training YAML
    # still supplies ordinary hyperparameters, but cannot silently resize the
    # small network.
    config["IDAAC_FC_DIM_SIZE"] = IDAAC_MATCHED_FC_DIM
    config["IDAAC_POLICY_HIDDEN_DIMS"] = IDAAC_MATCHED_POLICY_HIDDEN_DIMS
    config["IDAAC_VALUE_HIDDEN_DIMS"] = IDAAC_MATCHED_VALUE_HIDDEN_DIMS
    # env = jaxmarl.make(config["ENV_NAME"], **config["ENV_KWARGS"])
    ...
    def train(rng, model_params=None, resume_runner_state=None):
        ...
        init_hstate = ActorCriticRNN.initialize_carry(
            config["NUM_ENVS"], 2 * _idaac_fc_dim(config)
        )
        network_params = network.init(_rng, init_hstate, init_x)
        parameter_count = _count_policy_value_parameters(network_params)
        print(f"IDAAC policy/value parameters: {parameter_count:,}")
        uses_reference_architecture = (
            config["ENV_NAME"] == "overcooked"
            and config["CONV_NET"]
            and config["LSTM"]
            and config["ACTION_DIM"] == 6
            and config["FC_DIM_SIZE"] == 256
            and config["GRU_HIDDEN_DIM"] == 256
            and _idaac_fc_dim(config) == IDAAC_MATCHED_FC_DIM
            and _idaac_policy_hidden_dims(config)
            == IDAAC_MATCHED_POLICY_HIDDEN_DIMS
        ...
        if resume_runner_state is None:
            ...
            init_hstate = ActorCriticRNN.initialize_carry(
                config["NUM_ACTORS"], 2 * _idaac_fc_dim(config)
            )
        ...
        def _update_step(update_runner_state, unused):
            ...
            def eval_layout(eval_env, params, eval_rng):
                ...
                init_hstate = ActorCriticRNN.initialize_carry(
                    num_actors_eval, 2 * _idaac_fc_dim(config)
                )
            ...
            def eval_layout_xp_direction(eval_env, main_params, bc_params, eval_rng, main_agent_id):
                ...
                init_obs, init_state = jax.vmap(eval_env.reset, in_axes=(0,))(reset_rngs)
                init_hstate = ActorCriticRNN.initialize_carry(
                    num_eval_envs, 2 * _idaac_fc_dim(config)
                )
...
def main(config):
    ...
    config["model_name"] = "CEC_IDAAC_SMALL"
    ...
    if config['TRAIN_KWARGS']['finetune']:
        filepath = os.path.join(
            "ckpts",
            "idaac_small_finetune",
            config['ENV_KWARGS']['layout'],
            active_xpid,
        )
    else:
        filepath_base = f"ckpts/idaac_small/{config['ENV_NAME']}"
        if config["ENV_NAME"] == "overcooked":
    ...
    if wandb_resume_id:
        ...
        wandb.init(
            entity=config["ENTITY"],
            project=config["PROJECT"],
            tags=["IDAAC", "RNN", "SP", "small", "parameter-matched"],
            config=config,
            mode=config["WANDB_MODE"],
            name=f"IDAAC_small_gradient_{layout_name}_seed{config['SEED']}"
        )
    ...
    if _has_mid_ckpt:
        ...
        print(
            "Loading CEC-IDAAC-small checkpoint for finetuning: "
