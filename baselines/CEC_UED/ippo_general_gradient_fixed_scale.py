...
def make_train(
    ...
    if config["FIXED_TARGET_SCALE"] <= 0:
        raise ValueError("FIXED_TARGET_SCALE must be greater than zero")
    ...
    def train(rng, model_params=None, resume_runner_state=None):
        fixed_target_scale = jnp.asarray(
            config["FIXED_TARGET_SCALE"], dtype=jnp.float32
        )
        ...
        def _update_step(update_runner_state, unused):
            ...
            def _calculate_gae(traj_batch, last_val):
                # The critic predicts value / FIXED_TARGET_SCALE. Convert its
                # outputs back to raw reward scale before computing GAE.
                last_val_real = last_val * fixed_target_scale

                def _get_advantages(gae_and_next_value, transition):
                    gae, next_value = gae_and_next_value
                    done, value_scaled, reward = (
                        transition.global_done,
                        transition.value,
                        transition.reward,
                    )
                    value_real = value_scaled * fixed_target_scale
                    delta = (
                        reward
                ...
                targets_real = (
                    advantages + traj_batch.value * fixed_target_scale
                )
            ...
            # ── raw and fixed-scale value-target statistics ───────────────
            _targets_scaled = targets / fixed_target_scale

            target_stats = {}
            target_stats["target_raw/mean"] = targets.mean()
            target_stats["target_fixed_scale/mean"] = _targets_scaled.mean()

            _err_scaled = _targets_scaled - traj_batch.value
            target_stats["critic/rmse"] = jnp.sqrt(
                (_err_scaled ** 2).mean()
            )
            ...
            for _lid, _name in enumerate(_LAYOUT_NAMES):
                ...
                _target_scaled_mean = (
                    (_targets_scaled * _mask).sum() / _cnt
                )
                _target_scaled_var = (
                    (
                        (_targets_scaled - _target_scaled_mean) ** 2
                        * _mask
                    ).sum()
                    / _cnt
                )
                target_stats[
                    f"target_fixed_scale/{_name}/mean"
                ] = jnp.where(_valid, _target_scaled_mean, jnp.nan)
                target_stats[
                    f"target_fixed_scale/{_name}/std"
                ] = jnp.where(
                    _valid, jnp.sqrt(_target_scaled_var), jnp.nan
                )
                target_stats[f"critic/{_name}/rmse"] = jnp.where(
                    _valid,
                    jnp.sqrt(
                        ((_err_scaled ** 2) * _mask).sum() / _cnt
                    ),
                    jnp.nan,
                )
            ...
            def _update_epoch(update_state, unused):
                def _update_minbatch(train_state, batch_info):
                    ...
                    def _loss_fn(params, init_hstate, traj_batch, gae, targets):
                        ...
                        # CALCULATE VALUE LOSS (fixed normalized scale)
                        targets_scaled = targets / fixed_target_scale
                        value_pred_clipped = traj_batch.value + (
                            value - traj_batch.value
                        ).clip(-config["CLIP_EPS"], config["CLIP_EPS"])
                        value_losses = jnp.square(value - targets_scaled)
                        value_losses_clipped = jnp.square(
                            value_pred_clipped - targets_scaled
                        )
            ...
            if save_info is not None:
                ...
                def final_save_callback(params):
                    ...
                    ckpt_path = f"{fp}/{prefix}seed{config['SEED']}_ckpt{config['TRAIN_KWARGS']['ckpt_id']}{appendage}_fixed_scale_updates{num_updates_total}.pkl"
...
def main(config):
    ...
    config["model_name"] = "CEC_FIXED_SCALE"
    ...
    if wandb_resume_id:
        ...
        wandb.init(
            ...
            config=config,
            mode=config["WANDB_MODE"],
            name=f"CEC_gradient_fixed_scale_{layout_name}_seed{config['SEED']}"
        )
