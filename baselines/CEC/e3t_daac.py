        ...
        def _update_step(update_runner_state, unused):
            ...
            def callback(metric):
                ...
                if current_return > best_return[0]:
                    ...
                    ckpt_path = f"{config['filepath']}/{config['fcp_prefix']}seed{config['SEED']}_best_e3t_daac.pkl"
...
def main(config):
    ...
    wandb.init(
        ...
        tags=["E3T", "DAAC", "RNN", "SP"],
        config=config,
        mode=config["WANDB_MODE"],
        name=f"e3t_daac_{config['ENV_KWARGS']['layout']}_seed{config['SEED']}"
    )
    filepath = f"ckpts/e3t_daac/{config['ENV_NAME']}"
    if config["ENV_NAME"] == "overcooked":
