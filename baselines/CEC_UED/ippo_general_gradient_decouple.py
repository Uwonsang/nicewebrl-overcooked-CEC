...
# The same diagnostic groups used by IPPO, mapped to the two independent
# parameter trees introduced by the decoupled architecture.
ACTOR_TRUNK_KEYS = (
    "actor_trunk", "actor_hidden_0", "actor_hidden_1",
    "actor_hidden_2", "actor_hidden_3", "actor_output",
)
VALUE_TRUNK_KEYS = (
    "critic_trunk", "critic_hidden_0", "critic_hidden_1",
    "critic_hidden_2", "critic_hidden_3", "critic_output",
)
...
class ActorCriticRNN(nn.Module):
    ...
    def __call__(self, hidden, x):
        ...
        actor_hidden, actor_embedding = RecurrentFeatureTrunk(
            config=self.config, role="actor", name="actor_trunk"
        )(actor_hidden, obs, dones)
        critic_hidden, critic_embedding = RecurrentFeatureTrunk(
            config=self.config, role="critic", name="critic_trunk"
        ...
        return ((actor_hidden, critic_hidden), pi,
...
def ppo_loss(
    ...
    """Standard PPO objective for the decoupled actor/critic network."""
...
def make_train(
    ...
    config.setdefault("DECOUPLED_POLICY_LR", config["LR"])
    config.setdefault("DECOUPLED_VALUE_LR", config["LR"])
    ...
    def train(rng, model_params=None, resume_runner_state=None):
        ...
        def optimizer(learning_rate):
            schedule = (
                functools.partial(linear_schedule, initial_lr=learning_rate)
                if config["ANNEAL_LR"] else learning_rate
            )
            return optax.chain(
        ...
        tx = optax.multi_transform(
            {
                "policy": optimizer(config["DECOUPLED_POLICY_LR"]),
                "value": optimizer(config["DECOUPLED_VALUE_LR"]),
            },
            param_labels,
        )
        ...
        def _update_step(update_runner_state, unused):
            ...
            advantages, targets, td_errors = _calculate_gae(
                traj_batch, last_val
            )
            ...
            # Per-step layout id, classified before env.step and already tiled
            # to actors.
            _actor_layout_full = traj_batch.layout_id
            _layout_ids_full = _actor_layout_full[:, :config["NUM_ENVS"]]
            ...
            def _compute_layout_gradient(_):
                layout_gradient_traj = jax.tree.map(
                    lambda x: x[:layout_gradient_window_steps], traj_batch
                )
            ...
            # Use the actor subset consumed by the first epoch's first PPO
            # minibatch, preserving each actor's matching recurrent state.
            ...
            def _update_epoch(update_state, unused):
                def _update_minbatch(train_state, batch_info):
                    ...
                    return train_state, (
                        loss, loss_aux, optimizer_update_metrics
                    )
            ...
            def eval_layout(eval_env, params, eval_rng):
                ...
                init_hstate = ActorCriticRNN.initialize_carry(
                    num_actors_eval, config["GRU_HIDDEN_DIM"]
                )
            ...
            def eval_layout_xp_direction(eval_env, main_params, bc_params, eval_rng, main_agent_id):
                ...
                init_obs, init_state = jax.vmap(eval_env.reset, in_axes=(0,))(reset_rngs)
                init_hstate = ActorCriticRNN.initialize_carry(
                    num_eval_envs, config["GRU_HIDDEN_DIM"]
                )
            ...
            def callback(metric):
                ...
                if (
                    ...
                    # Expensive diagnostics are evaluated only at this update
                    # and logged directly rather than interval-averaged.
...
def main(config):
    ...
    config["model_name"] = "CEC_DECOUPLED"
    config.setdefault("DECOUPLED_POLICY_LR", config["LR"])
    config.setdefault("DECOUPLED_VALUE_LR", config["LR"])
    if config["TRAIN_KWARGS"]["finetune"]:
        raise ValueError(
            "CEC shared checkpoints cannot be loaded directly into the "
            "decoupled actor/critic parameter tree. Train this model from "
            "scratch or add an explicit parameter-conversion step."
        )
    xpid = (
    ...
    filepath_base = f"ckpts/ippo_decouple/{config['ENV_NAME']}"
    if config["ENV_NAME"] == "overcooked":
    ...
    if wandb_resume_id:
        ...
        wandb.init(
            entity=config["ENTITY"],
            project=config["PROJECT"],
            tags=["CEC_DECOUPLED", "RNN", "SP"],
            config=config,
            mode=config["WANDB_MODE"],
            name=f"CEC_decoupled_{layout_name}_seed{config['SEED']}"
        )
