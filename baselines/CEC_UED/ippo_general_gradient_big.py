...
def _big_fc_dim(config):
    return int(config.get("IPPO_BIG_FC_DIM", IPPO_BIG_FC_DIM))


def _big_policy_hidden_dims(config):
    return tuple(
        int(width)
        for width in config.get(
            "IPPO_BIG_POLICY_HIDDEN_DIMS",
            IPPO_BIG_POLICY_HIDDEN_DIMS,
        )
    )


def _big_value_hidden_dims(config):
    return tuple(
        int(width)
        for width in config.get(
...
def _count_trainable_parameters(variables):
...
class ActorCriticRNN(nn.Module):
    ...
    def __call__(self, hidden, x):
        ...
        fc_dim = _big_fc_dim(self.config)
        policy_hidden_dims = _big_policy_hidden_dims(self.config)
        value_hidden_dims = _big_value_hidden_dims(self.config)
        ...
        embedding = nn.Dense(
            fc_dim * 2, kernel_init=orthogonal(np.sqrt(2)), bias_init=constant(0.0)
        )(embedding)
        embedding = nn.relu(embedding)
        ...
        if self.config["LSTM"]:
            ...
            embedding = nn.Dense(fc_dim * 2, kernel_init=orthogonal(2), bias_init=constant(0.0))(embedding)
            embedding = nn.relu(embedding)
        embedding = embedding.reshape((batch_size, num_envs, -1))
        ...
        # Actor
        #########
        actor_mean = nn.Dense(policy_hidden_dims[0], kernel_init=orthogonal(2), bias_init=constant(0.0))(
            embedding
        )
        actor_mean = nn.relu(actor_mean)
        record_feature_norm("actor_hidden_0", actor_mean)
        actor_mean = nn.Dense(policy_hidden_dims[1], kernel_init=orthogonal(2), bias_init=constant(0.0))(
            actor_mean
        )
        actor_mean = nn.relu(actor_mean)
        record_feature_norm("actor_hidden_1", actor_mean)
        actor_mean = nn.Dense(
            policy_hidden_dims[2], kernel_init=orthogonal(2), bias_init=constant(0.0)
        )(actor_mean)
        actor_mean = nn.relu(actor_mean)
        record_feature_norm("actor_hidden_2", actor_mean)
        if self.config["ENV_NAME"] == "overcooked":
            actor_mean = nn.Dense(policy_hidden_dims[3], kernel_init=orthogonal(2), bias_init=constant(0.0))(
                actor_mean
            )
        ...
        # Critic
        #########
        critic = nn.Dense(value_hidden_dims[0], kernel_init=orthogonal(2), bias_init=constant(0.0))(
            embedding
        )
        critic = nn.relu(critic)
        record_feature_norm("critic_hidden_0", critic)
        critic = nn.Dense(value_hidden_dims[1], kernel_init=orthogonal(2), bias_init=constant(0.0))(
            critic
        )
        critic = nn.relu(critic)
        record_feature_norm("critic_hidden_1", critic)
        if self.config["ENV_NAME"] == "overcooked":
            critic = nn.Dense(value_hidden_dims[2], kernel_init=orthogonal(2), bias_init=constant(0.0))(
                critic
            )
            critic = nn.relu(critic)  # extra layer 1
            record_feature_norm("critic_hidden_2", critic)
            critic = nn.Dense(value_hidden_dims[3], kernel_init=orthogonal(2), bias_init=constant(0.0))(
                critic
            )
...
def make_train(
    ...
    # Keep the big architecture fixed even though ordinary training
    # hyperparameters continue to come from the shared YAML.
    config["IPPO_BIG_FC_DIM"] = IPPO_BIG_FC_DIM
    config["IPPO_BIG_POLICY_HIDDEN_DIMS"] = IPPO_BIG_POLICY_HIDDEN_DIMS
    config["IPPO_BIG_VALUE_HIDDEN_DIMS"] = IPPO_BIG_VALUE_HIDDEN_DIMS
    # env = jaxmarl.make(config["ENV_NAME"], **config["ENV_KWARGS"])
    ...
    def train(rng, model_params=None, resume_runner_state=None):
        ...
        init_hstate = ScannedRNN.initialize_carry(
            config["NUM_ENVS"], 2 * _big_fc_dim(config)
        )
        network_params = network.init(_rng, init_hstate, init_x)
        parameter_count = _count_trainable_parameters(network_params)
        print(f"IPPO-big policy/value parameters: {parameter_count:,}")
        uses_reference_architecture = (
            config["ENV_NAME"] == "overcooked"
            and config["CONV_NET"]
            and config["LSTM"]
            and config["ACTION_DIM"] == 6
            and _big_fc_dim(config) == IPPO_BIG_FC_DIM
            and _big_policy_hidden_dims(config)
            == IPPO_BIG_POLICY_HIDDEN_DIMS
        ...
        if resume_runner_state is None:
            ...
            init_hstate = ScannedRNN.initialize_carry(
                config["NUM_ACTORS"], 2 * _big_fc_dim(config)
            )
        ...
        def _update_step(update_runner_state, unused):
            ...
            def eval_layout(eval_env, params, eval_rng):
                ...
                init_hstate = ScannedRNN.initialize_carry(
                    num_actors_eval, 2 * _big_fc_dim(config)
                )
            ...
            def eval_layout_xp_direction(eval_env, main_params, bc_params, eval_rng, main_agent_id):
                ...
                init_obs, init_state = jax.vmap(eval_env.reset, in_axes=(0,))(reset_rngs)
                init_hstate = ScannedRNN.initialize_carry(
                    num_eval_envs, 2 * _big_fc_dim(config)
                )
...
def main(config):
    ...
    config["model_name"] = "CEC_IPPO_BIG"
    config["IPPO_BIG_FC_DIM"] = IPPO_BIG_FC_DIM
    config["IPPO_BIG_POLICY_HIDDEN_DIMS"] = IPPO_BIG_POLICY_HIDDEN_DIMS
    config["IPPO_BIG_VALUE_HIDDEN_DIMS"] = IPPO_BIG_VALUE_HIDDEN_DIMS
    ...
    if config['TRAIN_KWARGS']['finetune']:
        ...
        filepath_base = f"ckpts/ippo_big/{config['ENV_NAME']}"
        if config["ENV_NAME"] == "overcooked":
    ...
    if wandb_resume_id:
        ...
        wandb.init(
            entity=config["ENTITY"],
            project=config["PROJECT"],
            tags=["IPPO", "RNN", "SP", "big", "parameter-matched"],
            config=config,
            mode=config["WANDB_MODE"],
            name=f"CEC_big_gradient_{layout_name}_seed{config['SEED']}"
        )
    ...
    if _has_mid_ckpt:
        ...
        print(f"Loading CEC-IPPO-big checkpoint for finetuning: {finetune_checkpoint}")
