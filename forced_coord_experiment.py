...
def initialize_environment(config):
  ...
  if config["ENV_NAME"] == "overcooked":
    ...
    def reset_env(key):
      def reset_sub_dict(key, fn):
        ...
        temp_o, temp_s = env.custom_reset(
          key, layout=sampled_layout_dict, random_reset=False, shuffle_inv_and_pot=False
        )
        key, subkey = jax.random.split(key)
    ...
    for (
      layout_name,
      layout_dict,
    ) in overcooked_layouts.items():  # add hand crafted ones to heldout set
      if "9" in layout_name:
...
instruction_block = Block(
  ...
  randomize=False,
  name="forced_coord_instructions",
)
...
for model_name, model in model_dict.items():
  environment_stage = MultiAgentEnvStage(
    name=f"{model_name}_forced_coord",
    web_env=jax_web_env,
    action_keys=action_keys,
    action_to_name=action_to_name,
    env_params=env_params,
  ...
  survey_stage = make_survey_stage(f"{model_name} Forced Coordination 설문")
  ...
  env_block = Block(
    ...
    randomize=False,
    name=f"forced_coord_{model_name}_block",
  )
