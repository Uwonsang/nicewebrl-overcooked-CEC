...
class PLRManager:
    ...
    def _get_next_insert_idx(self, plr_buffer):
        return jax.lax.cond(
            ...
            lambda *_: jnp.argmin(self._get_replay_dist(plr_buffer.scores,
                                  plr_buffer.ages, plr_buffer.filled))
        )
    ...
    def _sample_buffer_uniform(self, rng, plr_buffer, n):
        p = plr_buffer.filled.astype(jnp.float32)
    ...
    def sample(self, rng, plr_buffer, new_levels, n, random=False):
        ...
        is_replay = jnp.greater(
            self.replay_prob, jax.random.uniform(replay_rng))
    ...
    def dedupe_levels(self, plr_buffer, levels, level_idxs):
        if self.comparator_fn is not None and level_idxs.shape[-1] > 2:
            def _check_equal(carry, step):
                ...
                matches = jax.vmap(self.comparator_fn, in_axes=(
                    0, None))(other_levels, level)

                top2match, top2match_idxs = jax.lax.top_k(matches, 2)

                is_self_dupe = jnp.logical_and(
                    is_self, top2match[1])  # More than 1 match
                is_dedupe_idx = jnp.logical_and(
                    is_self_dupe, jnp.greater(batch_idx, top2match_idxs[0]))
                self_match_idx = top2match_idxs[0] * \
                    is_dedupe_idx - (~is_dedupe_idx)
    ...
    def update(self, plr_buffer, levels, level_idxs, ued_scores, dupe_mask=None, info=None, ignore_val=-jnp.inf, parent_idxs=None):
        ...
        def update_level_info(carry, step):
            ...
            should_insert = jnp.greater_equal(
                score, scores.at[insert_idx].get())
            ...
            should_insert_or_update = jnp.logical_or(
                should_insert, should_update)
        ...
        carry = (ued_scores, levels, level_idxs,
                 done_masks, parent_idxs, max_returns)
